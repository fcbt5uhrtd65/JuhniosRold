...
import { Download, FileText, FileUp, KeyRound, Save, User, Wallet, X } from 'lucide-react';
...
type ProfileTab = 'personal' | 'social' | 'banking' | 'emergency' | 'documents' | 'payslips' | 'access' | 'history';
...
export function EmployeeSelfProfileModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([getMyEmployeeProfile(), getMyEmployeeDocuments(), getMyPayslipDocuments({ limit: 24 })]).then(([profileRes, documentsRes, payslipsRes]) => {
      if (cancelled) return;
      ...
      setPayslips(payslipsRes.status === 'fulfilled' ? payslipsRes.value.data : []);
      setLoading(false);
    });
    return () => {
      cancelled = true;
                ...
                <div className="space-y-4">
                  <div className="grid sm:grid-cols-3 gap-3">
                    ...
                    <Card className="p-4 sm:col-span-2">
                      <div className="text-[11px] font-semibold uppercase tracking-wider text-gray-500 mb-1">Último periodo publicado</div>
                      <div className="text-sm font-semibold text-gray-900">
                        {payslips[0] ? `${parseDate(payslips[0].period_start)} - ${parseDate(payslips[0].period_end)}` : 'Sin volantes disponibles'}
                      </div>
                    </Card>
                  </div>
                  ...
                  <div className="grid lg:grid-cols-2 gap-3">
                    {payslips.map((payslip) => (
                      <div key={payslip.id} className="border border-gray-100 rounded-xl p-4 bg-white shadow-sm">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 text-sm font-semibold text-gray-900">
                              <FileText size={15} className="text-[#2a4038] flex-shrink-0" />
                              <span className="truncate">{payslip.title}</span>
                            </div>
                            <div className="mt-1 text-xs text-gray-500">
                              {parseDate(payslip.period_start)} - {parseDate(payslip.period_end)}
                            </div>
                            <div className="text-xs text-gray-400">Pago: {parseDate(payslip.payment_date)}</div>
                          </div>
                          <Badge label="Publicado" color="green" />
                        </div>
                        {payslip.notes && <p className="mt-3 text-xs text-gray-500 line-clamp-2">{payslip.notes}</p>}
                        <button
                          type="button"
                          onClick={() => void handlePayslipDownload(payslip)}
                          disabled={downloadingPayslipId === payslip.id}
                          className="mt-4 w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-[#2a4038] text-white text-xs font-semibold hover:bg-[#3d5c4e] disabled:opacity-50"
                        >
                          <Download size={14} />
                          {downloadingPayslipId === payslip.id ? 'Descargando...' : 'Descargar volante'}
                        </button>
                      </div>
                    ))}
                  </div>
                  {payslips.length === 0 && <EmptyState title="Aún no tienes volantes de pago publicados." />}
                </div>
              )}
