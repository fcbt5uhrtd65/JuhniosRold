...
def draw_letterhead_header(c, page_w, page_h, x0, x1):
    """Dibuja la franja decorativa oficial en la parte superior de la página, a todo el
    ancho del contenido. Devuelve la coordenada y donde puede comenzar el resto del encabezado."""
    ...
    y = page_h - h
    try:
        c.drawImage(image, x0, y, width=w, height=h, preserveAspectRatio=True, mask="auto")
    except Exception:
        return top_y
    return y - 16
...
def draw_letterhead_footer(c, page_w, x0, x1):
    """Dibuja la franja de pie de página oficial (contacto) a todo el ancho del contenido,
    anclada al borde inferior de la hoja. Devuelve la altura ocupada (para reservar margen inferior)."""
    ...
    h = w * (ih / iw) if iw else 0
    try:
        c.drawImage(image, x0, 0, width=w, height=h, preserveAspectRatio=True, mask="auto")
    except Exception:
        return 0
    return h
