...
def sync_payroll_components(apps, schema_editor):
    """Crea los componentes ``human_resources.payroll`` y
    ``human_resources.biometric_import`` (ausentes hasta ahora) y resincroniza
    la matriz completa de permisos, siguiendo el mismo patrón que
    0015_sync_loans_component.py — separa la nómina y la importación de
    marcaciones biométricas de la gestión general de RRHH
    (human_resources.management), con roles específicos por función."""
...
class Migration(migrations.Migration):
    dependencies = [
        ("identity", "0015_sync_loans_component"),
    ]

    operations = [
        migrations.RunPython(sync_payroll_components, migrations.RunPython.noop),
    ]
