...
def sync_loans_component(apps, schema_editor):
    """Crea el componente ``human_resources.loans`` (ausente hasta ahora) y
    resincroniza la matriz completa de permisos para que quede visible y
    editable desde el panel de Permisos — antes el acceso de Contabilidad y
    Tesorería a préstamos vivía oculto en código (can_view_loans/_resolver_role)
    sin ningún componente real que lo representara en la UI de administración."""
...
class Migration(migrations.Migration):
    dependencies = [
        ("identity", "0014_sync_tesoreria_role"),
    ]

    operations = [
        migrations.RunPython(sync_loans_component, migrations.RunPython.noop),
    ]
